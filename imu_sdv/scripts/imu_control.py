...
class ImuControl:
    ...
    def imu_odom(self):
        if self.imu_longitude!=0 and self.imu_latitude!=0 and self.enable_imu==False:
            new_x=(self.imu_longitude-self.origin_x )*self.threshold
            new_y=(self.imu_latitude-self.origin_y)*self.threshold
            current_coord=str(new_x)+" "+str(new_y)+" "+"0 0 0 0 map odom"
